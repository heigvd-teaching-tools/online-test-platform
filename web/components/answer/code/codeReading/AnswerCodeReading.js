...
const AnswerCodeReading = ({ evaluationId, questionId, question, answer, onAnswerChange }) => {
  ...
  const onOutputCheck = useCallback(
    async () => {
      setLockCodeReadingCheck(true);
      ...
      const newOutputs = outputs.map((output) => {
        const result = results.find((r) => r.codeReadingSnippet.order === output.codeReadingSnippet.order);
        return {
          ...output,
  ...
  return (
    ...
    { studentOutputTest && (
      ...
      {studentOutputTest && (
        <Stack alignItems={"flex-start"} py={2} px={1} borderTop={`1px solid ${theme.palette.divider}`} bgcolor={theme.palette.background.paper}>
          <Button
