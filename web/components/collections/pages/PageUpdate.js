...
const PageUpdate = () => {
  ...
  const onQuestionChange = useCallback(
    async (questionId, changedProperties) => {
  ...
  const createQuestion = useCallback(async () => {
    await fetch(`/api/exams/${router.query.examId}/questions`, {
      ...
      .then(async (createdQuestion) => {
        ...
        await router.push(
          `/exams/${router.query.examId}/questions/${questions.length + 1}`
        )
      })
      .catch(() => {
  ...
  const updateQuestionOrder = useCallback(
    async (order, offset) => {
      ...
      await savePositions()
      await router.push(
        `/exams/${router.query.examId}/questions/${order + offset + 1}`
      )
    },
    [mutate, savePositions, questions, router]
  )

  const orderDecrease = useCallback(
    async (order) => await updateQuestionOrder(order, -1),
    [updateQuestionOrder]
  )
  const orderIncrease = useCallback(
    async (order) => await updateQuestionOrder(order, 1),
    [updateQuestionOrder]
  )
            ...
            <Stack direction="row" alignItems="center">
              ...
              {questions && (
                <QuestionPages
                  questions={questions}
                  activeQuestion={
                    questions[parseInt(router.query.questionIndex) - 1]
                  }
                  link={(_, index) => {
                    return `/exams/${router.query.examId}/questions/${
                      index + 1
                    }`
                  }}
                />
          ...
          {questions &&
            ...
            questions.map((q, index) => (
              <Box
                ...
                height="100%"
                display={
                  index + 1 === parseInt(router.query.questionIndex)
                    ? 'block'
                    : 'none'
                }
              >
