...
const CollectionToQuestion = ({ collectionToQuestion, index, onChange, onDelete }) => {

    const deleteCollectionToQuestion = useCallback(async (toDelete) => {
        // delete collectionToQuestion
        // mutate collection
        const response = await fetch(`/api/collections/${toDelete.collectionId}/questions`, {
            method: 'DELETE',
            headers: {
    ...
    const saveCollectionToQuestion = useCallback(async (index, updated) => {
        // save collectionToQuestion
        const response = await fetch(`/api/collections/${updated.collectionId}/questions`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const debounceSaveCollectionToQuestion = useDebouncedCallback(saveCollectionToQuestion, 300);
    ...
    return (
        <Paper variant={"outlined"}>
            <Stack direction="row" alignItems="center" spacing={1} pr={1}>
                ...
                <IconButton key="delete-collection" onClick={async (ev) => {
                    ev.preventDefault();
                    ev.stopPropagation();
