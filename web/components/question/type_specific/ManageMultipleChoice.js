...
import { Alert, AlertTitle, Button, FormControlLabel, FormGroup, FormHelperText, Switch, TextField, Typography } from '@mui/material'
  ...
  const onAddOption = useCallback(async () => {
    await fetch(
      `/api/${groupScope}/questions/${questionId}/multiple-choice/options`,
      {
        ...
        body: JSON.stringify({
          option: { text: '', isCorrect: false, order: multipleChoice?.options.length },
        }),
      },
    )
      .then(async (res) => {
...
const MultipleChoiceConfig = ({ multipleChoice, onPropertyChange }) => {
  ...
  const [ activateStudentComment, setAllowStudentComment ] = useState(multipleChoice?.activateStudentComment)
  const [ studentCommentLabel, setStudentCommentLabel ] = useState(multipleChoice?.studentCommentLabel)

  const [ activateSelectionLimit, setActivateSelectionLimit ] = useState(multipleChoice?.activateSelectionLimit)
          ...
          <UserHelpPopper>
            <Stack spacing={1} p={1}>
              <Alert severity="info" variant="outlined">
                ...
                An input field will be displayed to the student to provide an explanation for their answer.
                </Typography>
              </Alert>
              <Alert severity="info" variant="outlined">
                <AlertTitle>Student comment label</AlertTitle>
                <Typography variant="body2">
                The label of the input field where the student will provide their explanation.
                </Typography>
              </Alert>
            </Stack>
          </UserHelpPopper>
          ...
          <UserHelpPopper>
            <Stack spacing={1} p={1}>
              <Alert severity="info" variant="outlined">
                ...
                <Typography variant="body2">
                The student wont be able to select more than the number of correct options.
                </Typography>
                <Typography variant="body2">
