...
const post = async (req, res) => {
    ...
    if(!questions || questions && questions.length === 0){
        res.status(400).json({ message: 'You collections session has no questions. Please select the reference collections.' });
        return;
    }

    if(label.length === 0){
        res.status(400).json({ message: 'You collections session has no label. Please enter a label.' });
        return;
    }
    ...
    try {
        ...
        // create the copy of code questions for the collections session
        ...
        switch(e.code){
            case 'P2002':
                ...
                break;
            default:
                res.status(500).json({ message: 'Error while updating collections session' });
                break;
        }
    }
}
